    ...
    try {
      ...
      if (!apiKey) {
        return {
          success: false,
          code: 'NO_KEY',
          error: `${provider.toUpperCase()} API key is not set.`,
          provider,
        };
      }
      ...
      console.log(
        `[Translate] START provider=${provider} video=${videoPath} stream=${streamIndex} codec=${sourceCodec} -> ${targetLang}`
      );
      ...
      const translated = await translator.translateSrt(
        srtText,
        langName,
        provider,
        apiKey,
