...
module.exports = function registerFileControl() {
  ...
  ipcMain.handle('file:createSerie', async (event, { serieName, metadata }) => {
    ...
    if (fs.existsSync(fullPath))
      return { success: false, message: 'Bu dizi zaten var!', path: fullPath };
  ...
  ipcMain.handle('file:createSeason', (event, data) => {
    const MEDIA_DIR = getMediaDir();
    if (!MEDIA_DIR || !fs.existsSync(MEDIA_DIR))
      return { isExist: false, message: 'Arşiv klasörü ayarlanmamış.' };
  ...
  ipcMain.handle('file:addEpisode', (event, data) => {
    ...
    eq.addVideos(data.videos.map((f) => ({ ...f, filePath: f.path, destFolder: fullPath, event })));
  ...
  ipcMain.handle('file:getSeriesDetail', async (event, folderName) => {
    ...
    try {
      ...
      const seasons = items
        .filter(
          (item) =>
            item.startsWith('Season') && fs.statSync(path.join(seriePath, item)).isDirectory()
        )
        .sort((a, b) => {
  ...
  ipcMain.handle('file:syncDatabase', async () => {
    const MEDIA_DIR = getMediaDir();
    if (!MEDIA_DIR) {
      return { success: false, error: 'Medya klasörü ayarlanmamış' };
    }
