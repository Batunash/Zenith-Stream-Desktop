...
vi.mock('../utils/decisionEngine', () => ({ decideProcessingStrategy: decideProcessingStrategyMock }));
...
require.cache[_analyzerPath] = { id: _analyzerPath, filename: _analyzerPath, loaded: true, exports: { analyzeFile: analyzeFileMock },                   children: [], paths: [] };
require.cache[_decisionPath] = { id: _decisionPath, filename: _decisionPath, loaded: true, exports: { decideProcessingStrategy: decideProcessingStrategyMock }, children: [], paths: [] };
require.cache[_builderPath]  = { id: _builderPath,  filename: _builderPath,  loaded: true, exports: { processVideo: processVideoMock },                  children: [], paths: [] };
...
require.cache[_eqPath]  = { id: _eqPath,  filename: _eqPath,  loaded: true, exports: episodeQueueMock,                    children: [], paths: [] };
require.cache[_imgPath] = { id: _imgPath, filename: _imgPath, loaded: true, exports: { downloadImage: downloadImageMock }, children: [], paths: [] };
...
describe('fileControl', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(handleSettings, 'getSettings').mockReturnValue({ MEDIA_DIR: 'C:\\Media', TMDB_API_KEY: 'testkey' });
    ...
    episodeQueueMock.mockImplementation(function () { return { addVideos: eqAddVideosSpy }; });
  ...
  describe('file:createSerie', () => {
    it('creates a directory and database entry on success', async () => {
      ...
      expect(fs.mkdirSync).toHaveBeenCalledWith(expect.stringContaining('Test Show'), { recursive: true });
  ...
  describe('file:deleteSerie', () => {
    it('deletes folder on success', async () => {
      ...
      expect(fs.rmSync).toHaveBeenCalledWith(expect.stringContaining('mock_exist'), { recursive: true, force: true });
      expect(result.success).toBe(true);
    });
  });
  ...
  describe('file:createSeason', () => {
    it('creates a season directory', async () => {
      ...
      expect(fs.mkdirSync).toHaveBeenCalledWith(expect.stringContaining('Season 2'), { recursive: true });
  ...
  describe('file:fetchMetadata', () => {
    ...
    it('returns error if no API key', async () => {
      vi.spyOn(handleSettings, 'getSettings').mockReturnValue({ MEDIA_DIR: 'C:\\Media', TMDB_API_KEY: '' });
      registerFileControl();
      const handler = ipcMain.handle.mock.calls.find(c => c[0] === 'file:fetchMetadata')[1];
  ...
  describe('file:createSerie — branches', () => {
    ...
    it('downloads an http poster and sets localPoster when imageDest exists', async () => {
      fs.existsSync.mockImplementation((p) => p === 'C:\\Media' || (typeof p === 'string' && p.endsWith('poster.jpg')));
      registerFileControl();
      const result = await createSerie()({}, { serieName: 'Show', metadata: { image: 'http://x/p.jpg', numberOfSeasons: 0 } });
      expect(downloadImageMock).toHaveBeenCalledWith('http://x/p.jpg', expect.stringContaining('poster.jpg'));
    ...
    it('uses the .jpg fallback when the http image has no extension', async () => {
      ...
      expect(downloadImageMock).toHaveBeenCalledWith('http://x/poster', expect.stringContaining('poster.jpg'));
    });
    ...
    it('copies a local image when metadata.image is an existing local path', async () => {
      ...
      expect(fs.copyFileSync).toHaveBeenCalledWith('mock_exist_img.jpg', expect.stringContaining('poster'));
    });
    ...
    it('still returns success:true when db.syncFilesystemToDatabase throws (inner try/catch)', async () => {
      vi.spyOn(db, 'syncFilesystemToDatabase').mockImplementation(() => { throw new Error('db down'); });
    ...
    it('returns success:false + error when mkdirSync throws (outer catch)', async () => {
      fs.mkdirSync.mockImplementationOnce(() => { throw new Error('denied'); });
      registerFileControl();
    ...
    it('logs and continues when the image download rejects (image catch)', async () => {
      ...
      const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
      registerFileControl();
      const result = await createSerie()({}, { serieName: 'ImgFail', metadata: { image: 'http://x/p.jpg' } });
  ...
  describe('file:createSeason — branches', () => {
    const createSeason = () => ipcMain.handle.mock.calls.find(c => c[0] === 'file:createSeason')[1];
  ...
  describe('file:addEpisode', () => {
    ...
    it('returns ok:false when MEDIA_DIR is not configured', async () => {
      vi.spyOn(handleSettings, 'getSettings').mockReturnValue({ MEDIA_DIR: '' });
      registerFileControl();
      const result = await addEpisode()({}, { serieName: 'Show', seasonId: 'Season 1', videos: [] });
    ...
    it('creates the folder when it does not exist and enqueues the videos', async () => {
      registerFileControl();
      const result = await addEpisode()({}, { serieName: 'Show', seasonId: 'Season 1', videos: [{ path: '/v.mp4' }] });
    ...
    it('skips mkdir when the season folder already exists', async () => {
      fs.existsSync.mockImplementation((p) => p === 'C:\\Media' || (typeof p === 'string' && p.includes('mock_exist')));
      registerFileControl();
      const result = await addEpisode()({}, { serieName: 'mock_exist', seasonId: 'Season 1', videos: [] });
  ...
  describe('file:getSeries — branches', () => {
    ...
    it('sets fullPosterPath when a series metadata has localPoster', async () => {
      fs.existsSync.mockImplementation((p) => p === 'C:\\Media' || (typeof p === 'string' && p.endsWith('metadata.json')));
  ...
  describe('file:getSeriesDetail — branches', () => {
    const getSeriesDetail = () => ipcMain.handle.mock.calls.find(c => c[0] === 'file:getSeriesDetail')[1];
    ...
    it('sets fullPosterPath when metadata has localPoster', async () => {
      fs.existsSync.mockImplementation((p) => p === 'C:\\Media' || (typeof p === 'string' && (p.endsWith('mock_exist') || p.endsWith('metadata.json'))));
      fs.readdirSync.mockReturnValue(['Season 1']);
    ...
    it('returns error when reading metadata throws', async () => {
      fs.existsSync.mockImplementation((p) => p === 'C:\\Media' || (typeof p === 'string' && (p.endsWith('mock_exist') || p.endsWith('metadata.json'))));
      fs.readFileSync.mockImplementation(() => { throw new Error('parse'); });
      registerFileControl();
      const result = await getSeriesDetail()({}, 'mock_exist');
  ...
  describe('file:deleteSerie — branches', () => {
    ...
    it('returns success:false with error when rmSync throws', async () => {
      fs.rmSync.mockImplementationOnce(() => { throw new Error('locked'); });
      registerFileControl();
  ...
  describe('file:deleteSeason — branches', () => {
    const deleteSeason = () => ipcMain.handle.mock.calls.find(c => c[0] === 'file:deleteSeason')[1];
  ...
  describe('file:deleteEpisode — branches', () => {
    const deleteEpisode = () => ipcMain.handle.mock.calls.find(c => c[0] === 'file:deleteEpisode')[1];
    ...
    it('returns success:false with error when unlinkSync throws', async () => {
      fs.unlinkSync.mockImplementationOnce(() => { throw new Error('busy'); });
      registerFileControl();
  ...
  describe('file:syncDatabase — branches', () => {
    const syncDatabase = () => ipcMain.handle.mock.calls.find(c => c[0] === 'file:syncDatabase')[1];
    ...
    it('returns success:false with error when db.syncFilesystemToDatabase throws', async () => {
      vi.spyOn(db, 'syncFilesystemToDatabase').mockImplementation(() => { throw new Error('db'); });
      registerFileControl();
      const result = await syncDatabase()({});
  ...
  describe('file:fetchMetadata — branches', () => {
    const fetchMetadata = () => ipcMain.handle.mock.calls.find(c => c[0] === 'file:fetchMetadata')[1];
    ...
    it('uses VITE_TMDB_API_KEY as a fallback when TMDB_API_KEY is empty', async () => {
      vi.spyOn(handleSettings, 'getSettings').mockReturnValue({ MEDIA_DIR: 'C:\\Media', VITE_TMDB_API_KEY: 'vitekey' });
      registerFileControl();
      axios.get
  ...
  describe('media:analyze/decide/process — error & progress branches', () => {
    ...
    it('media:process forwards percent via event.sender.send and returns the new path', async () => {
      ...
      const mockEvent = { sender: { send: vi.fn() } };
      const result = await handler(mockEvent, { filePath: 'file.mp4', userPreferences: {} });
      expect(mockEvent.sender.send).toHaveBeenCalledWith('media:progress', { filePath: 'file.mp4', percent: 42 });
    ...
    it('media:process returns success:false when processVideo rejects', async () => {
      ...
      const result = await handler({ sender: { send: vi.fn() } }, { filePath: 'file.mp4', userPreferences: {} });
  ...
  describe('final branch closures (5 branches + line 127)', () => {
    ...
    it('file:createSerie: non-http image that does not exist leaves localPoster null (line 51 else-false)', async () => {
      registerFileControl();
      const result = await createSerie()({}, { serieName: 'NoImg', metadata: { image: 'C:\\nope\\missing.jpg' } });
    ...
    it('file:createSerie: skips mkdir for a season folder that already exists (line 73 else-false)', async () => {
      ...
      registerFileControl();
      const result = await createSerie()({}, { serieName: 'Seasons', metadata: { numberOfSeasons: 1 } });
