...
describe('burnControl', () => {
    ...
    it('succeeds and sends progress', async () => {
        ...
        expect(event.sender.send).toHaveBeenCalledWith('media:burnExternalSubtitle:progress', { percent: 0 });
        expect(event.sender.send).toHaveBeenCalledWith('media:burnExternalSubtitle:progress', { percent: 50 });
        expect(event.sender.send).toHaveBeenCalledWith('media:burnExternalSubtitle:progress', { percent: 100 });
        expect(burner.burnExternalSrt).toHaveBeenCalledWith('test.mkv', 'test.srt', expect.any(Function));
    });
