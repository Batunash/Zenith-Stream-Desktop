  ...
  return new Promise((resolve, reject) => {
    if (sourceCodec && IMAGE_SUB_CODECS.has(sourceCodec)) {
      return reject(
        new SubtitleError(
          'IMAGE_SUB',
          `Image-based subtitle (${sourceCodec}) cannot be extracted as text.`
        )
      );
    }
    if (sourceCodec && !TEXT_SUB_CODECS.has(sourceCodec)) {
      return reject(
        new SubtitleError('UNSUPPORTED_SUB', `Unsupported subtitle codec: ${sourceCodec}`)
      );
    }
...
const buildBatches = (cues) => {
  ...
  for (const cue of cues) {
    ...
    if (
      current.length &&
      (chars + len > MAX_CHARS_PER_BATCH || current.length >= MAX_CUES_PER_BATCH)
    ) {
...
const callProvider = async (prompt, providerName, apiKey, emit) => {
  ...
  if (!providerCfg)
    throw new SubtitleError('UNKNOWN_PROVIDER', `Unknown provider: ${providerName}`);
  ...
  while (true) {
    ...
    try {
      ...
      if (status === 429 && attempt < MAX_RETRIES_429) {
        ...
        console.warn(
          `[Translate] ${providerCfg.label} ${model} 429 (attempt ${attempt}/${MAX_RETRIES_429}). Waiting ${Math.round(wait / 1000)}s. API: ${apiMsg}`
        );
      ...
      if (isTransient && modelIdx < models.length - 1) {
        modelIdx++;
        console.warn(
          `[Translate] ${providerCfg.label} ${model} failed (${status || code}), rotating to ${models[modelIdx]}. API: ${apiMsg}`
        );
        await sleep(2000);
        continue;
      }
      ...
      if (status === 401 || status === 403) {
        ...
        throw new SubtitleError(
          'INVALID_KEY',
          apiMsg || `${providerCfg.label} rejected the API key.`
        );
      }
      if (status === 429) {
        ...
        throw new SubtitleError(
          'RATE_LIMITED',
          apiMsg || `${providerCfg.label} rate limit hit. Try again later.`
        );
      }
      console.error(
        `[Translate] ${providerCfg.label} ${model} HTTP ${status || '?'} error: ${apiMsg || err.message}`
      );
...
const translateSrt = async (srtText, targetLang, providerName, apiKey, emit) => {
  ...
  console.log(
    `[Translate] ${providerCfg.label}: ${cues.length} cues -> ${batches.length} batches (target: ${targetLang}, gap: ${gapMs}ms)`
  );
  ...
  for (let bi = 0; bi < batches.length; bi++) {
    ...
    if (lastRequestAt && elapsed < gapMs) {
      ...
      console.log(
        `[Translate] Pacing ${Math.round(waitMs / 1000)}s before batch ${bi + 1}/${batches.length}`
      );
    ...
    console.log(
      `[Translate] Batch ${bi + 1}/${batches.length} (${slice.length} cues, ${slice.reduce((n, c) => n + c.text.length, 0)} chars)`
    );
    if (emit)
      emit({
        stage: 'translate',
...
const countSubtitleStreams = (videoPath) =>
  new Promise((resolve) => {
