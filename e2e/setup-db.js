...
async function setup() {
  ...
  const serieRes = db.prepare("INSERT INTO SERIES (TITLE, FOLDER_PATH) VALUES (?, ?)").run('Test Series', dummyPath);
  const seasonRes = db.prepare("INSERT INTO SEASONS (SERIE_ID, SEASON_NUMBER, NAME, FOLDER_PATH) VALUES (?, ?, ?, ?)").run(serieRes.lastInsertRowid, 1, 'Season 1', seasonPath);
  db.prepare("INSERT INTO EPISODES (SEASON_ID, EPISODE_NUMBER, NAME, FILE_PATH, FILE_SIZE, DURATION) VALUES (?, ?, ?, ?, ?, ?)").run(seasonRes.lastInsertRowid, 1, 'Ep 1', filePath, 0, 0);
