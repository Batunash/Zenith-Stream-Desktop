...
describe('DownloadManager', () => {
  ...
  describe('address bar navigation', () => {
    it('invokes browser:navigate on submit when the URL is non-empty', async () => {
      ...
      await waitFor(() =>
        expect(mockInvoke).toHaveBeenCalledWith('browser:navigate', 'http://target.com')
      );
    });
    ...
    it('updates the address bar when browser:urlChanged fires', async () => {
      ...
      expect(screen.getByPlaceholderText('downloadManager.urlPlaceholder').value).toBe(
        'http://from-ipc'
      );
    });
  });
  ...
  describe('stream detection', () => {
    it('shows the banner with a count when browser:getStreams returns streams', async () => {
      mockInvoke.mockImplementation(async (ch) => {
        if (ch === 'browser:getStreams')
          return { success: true, streams: [baseStream(1), baseStream(2)] };
        if (ch === 'browser:downloads') return [];
        return { success: true };
      });
      renderPage();
      await waitFor(() =>
        expect(screen.getByText(/streamCapture.detectedStreams/)).toBeInTheDocument()
      );
  ...
  describe('download modal', () => {
    const withSeries = async (
      series = [{ folderName: 'Series1', title: 'Series One' }],
      ...
      await waitFor(() =>
        expect(screen.getByText('downloadManager.modalTitle')).toBeInTheDocument()
      );
    };
    ...
    it('defaults to "Season 1" when the selected series has no seasons', async () => {
      ...
      fireEvent.change(screen.getByText('Series One').closest('select'), {
        target: { value: 'Series1' },
      });
      await waitFor(() =>
        expect(mockInvoke).toHaveBeenCalledWith('file:getSeriesDetail', 'Series1')
      );
    ...
    it('closes the modal on Cancel', async () => {
      ...
      await waitFor(() =>
        expect(screen.queryByText('downloadManager.modalTitle')).not.toBeInTheDocument()
      );
    });

    it('re-fetches seasons when the series select changes', async () => {
      await withSeries([
        { folderName: 'S1', title: 'A' },
        { folderName: 'S2', title: 'B' },
      ]);
      ...
      await waitFor(() =>
        expect(mockInvoke).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            ...
            libraryContext: expect.objectContaining({
              enabled: true,
              serieName: 'Series1',
              seasonId: 'Season 1',
            }),
    ...
    it('increments the episode name (01->02) and exposes the next name via the Quick-Save title', async () => {
      ...
      fireEvent.change(screen.getByPlaceholderText('downloadManager.episodePlaceholder'), {
        target: { value: 'Episode 01' },
      });
  ...
  describe('quick save', () => {
    it('returns early when there is no lastLibraryContext', async () => {
      ...
      expect(
        screen.queryByRole('button', { name: 'downloadManager.quickSave' })
    ...
    it('downloads with the pre-increment filename and advances lastEpisodeName', async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByRole('button', { name: 'downloadManager.quickSave' })
        ).toBeInTheDocument()
      );
      ...
      await waitFor(() =>
        expect(mockInvoke).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            filename: 'Ep 02',
            libraryContext: expect.objectContaining({ episodeName: 'Ep 02', enabled: true }),
          })
        )
      );
      expect(screen.getByRole('button', { name: 'downloadManager.quickSave' }).title).toContain(
        'Ep 03'
      );
    });
  });
  ...
  describe('downloads sidebar', () => {
    const openSidebarWith = async (downloads) => {
      ...
      await waitFor(() =>
        expect(screen.getByText('downloadManager.downloadQueue')).toBeInTheDocument()
      );
      return r;
    };
    ...
    it('renders a completed download with a green check and 100%', async () => {
      await openSidebarWith([
        { id: 1, title: 'Done', percent: 100, sizeKB: 0, status: 'completed' },
      ]);
    ...
    it('renders a downloading item with a Cancel (x) button', async () => {
      await openSidebarWith([
        { id: 3, title: 'Go', percent: 50, sizeKB: 0, status: 'downloading' },
      ]);
    ...
    it('uses the striped bar and shows MB for an in-progress item with unknown progress', async () => {
      const { container } = await openSidebarWith([
        { id: 5, title: 'Unknown', percent: 0, sizeKB: 2048, status: 'downloading' },
      ]);
    ...
    it('hides the sidebar when the close (x) button is clicked', async () => {
      ...
      await waitFor(() =>
        expect(screen.queryByText('downloadManager.downloadQueue')).not.toBeInTheDocument()
      );
    });
  });
  ...
  describe('IPC progress events', () => {
    it('refreshes downloads when browser:progress fires', async () => {
      ...
      await act(async () => {
        receiveCbs['browser:progress']?.();
      });
      await waitFor(() =>
        expect(
          mockInvoke.mock.calls.filter((c) => c[0] === 'browser:downloads').length
