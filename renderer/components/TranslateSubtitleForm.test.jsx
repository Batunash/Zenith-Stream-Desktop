...
describe('TranslateSubtitleForm Component', () => {
  ...
  it('renders warning if no text subtitles available', () => {
    render(<TranslateSubtitleForm analysis={{ subtitles: [{ index: 0, type: 'pgs', language: 'eng' }] }} />);
  ...
  it('renders form and can select languages', async () => {
    ...
    expect(window.api.invoke).toHaveBeenCalledWith('media:translateSubtitle', expect.objectContaining({
      targetLang: 'tr'
    }));
  });
  ...
  it('handles source selection change', () => {
    ...
    act(() => { fireEvent.change(sourceSelect, { target: { value: '' } }); });
    expect(sourceSelect.value).toBe("");

    act(() => { fireEvent.click(screen.getByRole('button', { name: /translate_btn/i })); });
    act(() => { fireEvent.click(screen.getByRole('button', { name: /translate_btn/i })); });
